...
namespace BeastInLabyrinth
{
    ...
    /// <summary>
    /// Represents the beast in the labyrinth.
    /// </summary>
    public class Beast
    {
    ...
    public class Labyrinth
    {
        ...
        private bool previousHadWallRight;
        ...
        public Labyrinth(int width, int height, List<string> mapLines)
        {
            ...
            this.previousHadWallRight = true;
            ...
            for (int row = 0; row < height; row++)
            {
                for (int col = 0; col < width; col++)
                {
                    ...
                    if (Direction.SymbolToDir.ContainsKey(c))
                    {
                        ...
                        beast = new Beast(row, col, direction);
                        mapData[row, col] = '.';
                        return;
                    }
                }
        ...
        public void PerformStep()
        {
            ...
            bool hasWallRight = !CanMoveTo(rightRow, rightCol);
            bool canGoForward = CanMoveTo(aheadRow, aheadCol);

            // Decision based on current and previous state
            if (previousHadWallRight && !hasWallRight)
            {
            ...
            previousHadWallRight = hasWallRight;
        }

        public void PrintMap()
        {
            var (beastRow, beastCol) = beast.GetPosition();
            char beastSymbol = beast.GetDirectionSymbol();

            for (int row = 0; row < height; row++)
            {
                for (int col = 0; col < width; col++)
                {
                    if (row == beastRow && col == beastCol)
                    {
                        Console.Write(beastSymbol);
                    }
                    else
                    {
